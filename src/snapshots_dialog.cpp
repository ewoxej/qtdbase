   ...
   : QDialog( parent ), m_patientItem( sel )
{
   ...
   setUiInit();
   QObject::connect( ui.pushButton_2, &QPushButton::clicked, this, &SnapshotsDialog::onAddPressed );
   QObject::connect( ui.pushButton, &QPushButton::clicked, this, &SnapshotsDialog::onDeletePressed );
}

void SnapshotsDialog::onAddPressed()
{
   QString fullPath = QFileDialog::getOpenFileName( this, "Choose snapshot" );
   if (fullPath == "") return;
   QString id = m_patientItem->indexes()[eDB_ID].data().toString();
   if (QPixmap( fullPath ).isNull())
   {
      QMessageBox pbmx;
      pbmx.setText( "File format not supported" );
      pbmx.exec();
      return;
   }
   QFile file( fullPath );
   ...
   QString newpath = "snapshots/" + id + "_" + QString::number( m_DbModel->rowCount() ) + "." + finfo.suffix();
...
void SnapshotsDialog::setUiInit()
{
   ...
   QObject::connect( sm, &QItemSelectionModel::selectionChanged, this, &SnapshotsDialog::onTableViewItemSelected );
}
